...
from Resnet18.utils import AverageMeter
...
def validation(model, dataloader, criterion):
    ...
    for batch_id, data in enumerate(dataloader):
        ...
        acc_meter.update(acc.cpu().numpy()[0], batch_size)
        if batch_id >0:
            print(f'-------Batch {batch_id},loss={loss_meter},acc={acc_meter}')
...
def main():
    ...
    val_dataloader = get_dataloader(train_dataset, mode = 'test', batch_size = batch_size)
    ...
    for epoch in range(1, total_epoch + 1):
        train_one_epoch(model,
                        ...
                        total_epoch)
        scheduler.step()
        validation(model, val_dataset, criterion)
